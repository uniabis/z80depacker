...
;	DEFINE	AVOID_SELFMODIFYING_CODE	2				; uncomment to disallow self-modifying code (-2 bytes, -4% speed)
...
@DecompressLZSA2:
	...
	ELSEIF	AVOID_SELFMODIFYING_CODE==2
		ld b,0 : scf : exa : jr ReadToken
	ELSE
		ld b,0 : push bc : scf : exa : jr ReadToken
	ENDIF
...
SaveOffset		push bc : pop ix : ld b,0
	ELSE
SaveOffset		inc sp : inc sp : push bc : ld b,0
	ENDIF
...
.PrevOffset		EQU $+1 : ld hl,0
	ELSEIF	AVOID_SELFMODIFYING_CODE==2
.useC			push ix : ex (sp),hl
	ELSE
.useC			ex (sp),hl : push hl
	ENDIF
			ADD_OFFSET
			COPY1
			COPYBC
...
VeryLongMatch:	ld c,(hl) : NEXT_HL
	...
	IFNDEF	AVOID_SELFMODIFYING_CODE
		ret
	ELSEIF	AVOID_SELFMODIFYING_CODE==2
		; but UNROLL_LONG_MATCHES uses self-modifing code
		ret
	ELSE
		pop af : ret
	ENDIF
